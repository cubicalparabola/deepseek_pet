...
export interface BubbleState {
  ...
  /**
   * 布局是否已经**定型**（行数已回报、气泡高度不会再变）。
   *
   * 为什么要这个标记：首次显示时主进程还不知道文本占几行，若此时就调整窗口，
   * 会先按"最大高度"把窗口撑大、气泡渲染一帧很高的样子，等行数回报后再收缩 ——
   * 用户看到的就是**显示气泡时闪一下**（逐帧诊断实测：窗口 resize 两次、
   * 气泡 419px 闪到 197px、宠物位置跳 400+px）。
   *
   * 因此 `ready === false` 时渲染层把气泡设为 `visibility: hidden`：
   * 尺寸照常计算（量行数需要），但不画出来。回报行数后主进程再定型并置 true。
   */
